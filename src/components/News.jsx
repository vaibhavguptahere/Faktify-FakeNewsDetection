...
const News = () => {
  ...
  const [searchTerm, setSearchTerm] = useState("latest news");
  ...
  const fetchNews = async () => {
    try {
      ...
      const response = await fetch(
        `https://newsapi.org/v2/top-headlines?q=${searchTerm}&category=${filter}&country=${country}&sortBy=${sortBy}&language=en&apiKey=${NEWS_API_KEY}`
      );
